...
The `panic` function yeets control out of the current function and up the call stack until it reaches a function that [defers a `recover`](https://go.dev/blog/defer-panic-and-recover). if not function calls `recover`, the goroutine (often the entire program) crashes.
...
func main() {
    ...
    // a truly astonishinly bad way to handle error
    enrichUser("123")
}
```

Sometimes new Go devs look at `panic/recover`, and think, "This is like `try/catch! I like this!" Don't be like them.

I use error values for all "normal" error handling, and if I have a truly unrecoverable error, I use [log.Fatal](https://golang.org/pkg/log/#Fatal) ot print a message and exit the program.

## Question 1

Panic and recover should be used instead of errrors...
...
If you want to cleanly exit in an unrecoverable way, which is a good alternative to panic?
