...
Challenge: 01-interfaces-in-go
Chapter: 05-interfaces

[Interfaces](https://go.dev/tour/methods/9) allow you to focus on what a type does rather than how it's built. They can help you write more flexible and reusuable code by defining behaviours (like methods) that different types can share. This makes it easy to swap out or update parts of your code without changing everything else.

Interfaces are just collections of method signatures. A type "implements" an interface if it has methods that match the interfaces's method signatures.
...
Notice that your code doesn't care at all whether a specific message is a `birthdayMessage` or a `sendingReport`!
