...
Challenge: 05-continue-break
Chapter: 07-loops

Whenever we want to change the the control flow of a loop we can use the `continue` and `break` keywords.
...
- We start checking at 2 because 1 isn't prime.

> [!WARNING]
> This lesson is graded based on the output of the program so don't leave any debug print statements.
...
We only want to print the numbers themselves, not the headings and delimiter, they are already handled in the test code.
